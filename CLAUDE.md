...
> 本文件 2026-08-21 重写,2026-08-25 随拆仓更新拓扑表述。旧版"双语链上版 / web3+AI 叠加层"身份已随 2026-08-13 线重启作废。
...
- 🔴 **共享 Cloudflare Worker(mathclass-ai)正本在本线**(2026-08-21 拍板):改 `worker/` 在本目录部署;改 `/api` 契约(现为 `{messages→text}`)前确认站内 3 处硬编码调用同步。Raccord 首发时会把 raccord 路由/CORS 合并进本线配置(见 raccord-deploy skill),届时别当成异物删掉。
...
走 **mathclass-deploy skill**(2026-08-21 按真实路径重写):发布源=本目录(`班级网站`),`MATHCLASS_PRIVATE_REPO=skip`,
